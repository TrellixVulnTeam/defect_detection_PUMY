...
def test_to_reports(sub_file, save_path, sheet, score_list, model_test_csv, ratio_array, table_header):
    ...
    for i, score in enumerate(score_list):
        ...
        xml2json = Xml2Labelme(os.path.dirname(csv_path), os.path.join(json_path, 'confidence_{}'.format(score)),
                               'result', score, 8)
        ...
        write_excel_xlsx_append(save_path, 'confidence', confidence, dic_writing_position[i][0] - 20,
                                dic_writing_position[i][1], sheet_name=sheet)  # 写入confidence
        ...
        write_excel_xlsx_append(save_path, 'zhibiao_header', header_zhibiao, dic_writing_position[i][0],
                                dic_writing_position[i][1], sheet_name=sheet)  # 写入指标
        write_excel_xlsx_append(save_path, 'biaoqian_header', [gt_cate[0:-1]], dic_writing_position[i][0],
                                dic_writing_position[i][1] + 1, sheet_name=sheet)  # 写入标签
        write_excel_xlsx_append(save_path, 'content', content, dic_writing_position[i][0] + 1,
                                dic_writing_position[i][1] + 1, sheet_name=sheet)  # 写入content
        add_louguojian_images(os.path.join(split_result_file, "confidence_" + str(score)), save_path, ratio_array,
                              dic_writing_position[i][0] + 10, dic_writing_position[i][1],
                              sheet_name=sheet)  # 插图：漏检and过检
...
def beautify_excel_content(i, dic_writing_position, excel_path):
    ...
    for sheet in sheet_names:
        for row in book['{}'.format(sheet)][
            'A{}:Z{}'.format(dic_writing_position[i][0] + 1, dic_writing_position[i][0] + 9)]:
            for cell in row:
...
def get_parser():
    parser = argparse.ArgumentParser(description='The tool used to generate the test report.')
...
if __name__ == '__main__':
    ...
    table_header = [['产品代号', args['product_name']],
                    ['光学面', args['optical_surface']],
                    ['模型版本号', args['model_ver']],
                    ['测试集版本号', args['validation_set_ver']],
                    ['测试通过条件', '刮伤漏检率<5%'],
                    ['', '黑点漏检率<2%'],
                    ['', '异物漏检率<1%'],
                    ['', ''],
    ...
    img_gt_path = args['ground_truth_path']  # img和gt_json路径
    model_test_csv = args['model_csv_path']  # csv路径
    score_list = args['score_list']  # confidence设置，可随意设置几个
    ratio_array = args['ratio_array']  # 插入[漏检, 过检]图片数量的百分比

    excel_save_path = args['output_path']  # 输出report路径
