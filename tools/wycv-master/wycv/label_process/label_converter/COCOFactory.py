...
class COCOFactory:
    ...
    def save_result(self):
        ...
        for img_item in self.image_list:
            img_process_pool.apply_async(self.img_process, args=(img_item, train_json, val_json,),
        ...
        for anno_item in annotation_list:
            anno_process_pool.apply_async(self.anno_process, args=(anno_item, train_json, val_json,),
                                          callback=lambda _: save_pbar.update())
        anno_process_pool.close()
        anno_process_pool.join()

        for json_item in [train_json, val_json]:
    ...
    def img_process(self, img_item, train_json, val_json):
    ...
    def anno_process(self, anno_item, train_json, val_json):
