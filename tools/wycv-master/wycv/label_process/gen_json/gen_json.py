...
def key_label_process(weakness_item) -> dict:
    try:
        weakness_item['label'] = pypinyin.slug(weakness_item['label'], separator='').split('.')[0]
    except:
        pass
    ...
    if weakness_item.get('describe'):
        weakness_item['describe'] = pypinyin.slug(weakness_item['describe'], separator='')
...
def process_bndbox(weakness_item):
    try:
        warning_list = []
        weakness_reorg_dict = {'label': pypinyin.slug(weakness_item.pop('name'), separator=''),
                               'shape_type': 'rectangle'}
        points_dict = weakness_item.pop('bndbox')
        points_dict = {k: float(v) for k, v in points_dict.items()}
        weakness_reorg_dict['points'] = [[points_dict['xmin'], points_dict['ymin']],
...
def process_point(weakness_item):
    try:
        warning_list = []
        weakness_reorg_dict = {'label': pypinyin.slug(weakness_item.pop('name'), separator=''), 'shape_type': 'circle'}
        points_dict = weakness_item.pop('point')
        points_dict = {k: float(v) for k, v in points_dict.items()}
        if not weakness_item.get('width'):
            weakness_item['width'] = 6
            warning_list.append(KeyError('width'))
        weakness_reorg_dict['points'] = [[points_dict['x'], points_dict['y']],
...
def process_ellipse(weakness_item):
    try:
        warning_list = []
        weakness_reorg_dict = {'label': pypinyin.slug(weakness_item.pop('name'), separator=''), 'shape_type': 'rectangle'}
...
def process_polygon(weakness_item):
    try:
        warning_list = []
        weakness_reorg_dict = {'label': pypinyin.slug(weakness_item.pop('name'), separator=''), 'shape_type': 'polygon'}
        if weakness_item['polygon'].get('points'):
            del weakness_item['polygon']['points']
        points_dict = weakness_item.pop('polygon')
        points_dict = {k: float(v) for k, v in points_dict.items()}
        weakness_reorg_dict['points'] = [list(points_dict.values())[i: i + 2] for i in range(0, len(points_dict), 2)]
        if weakness_item.get('width'):
            weakness_item.pop('width')
        weakness_item = key_label_process(weakness_item)
        weakness_reorg_dict.update(weakness_item)
...
def process_line(weakness_item):
    try:
        ...
        points_pair_list = [list(points_dict.values())[i: i + 2] for i in range(0, len(points_dict), 2)]
        ...
        if len(points_pair_list) == 1:
            weakness_item['point'] = {'x': points_pair_list[0][0], 'y': points_pair_list[0][1]}
            return process_point(weakness_item)
        else:
            weakness_reorg_dict['shape_type'] = 'line' if len(points_pair_list) == 2 else 'linestrip'
...
class GenLabelme:
    ...
    def get_data_list(work_dir_list) -> dict:
        ...
        if exception_list:
            print('WARNING: These folder contains more than 1 kind of format data, {}'.format(str(exception_list)))
        return data_list
    ...
    def xml_to_labelme(self, source_data_path: str, data_item: str):
        ...
        if os.path.isfile(data_abs_path) and data_suffix.lower() in configs.IMG_SUFFIX_LIST:
            try:
                labelme_json_out_file = os.path.join(self.output_path, data_id + '.json')
                ...
                img_name = data_id + '.jpg' if not self.disable_jpg else os.path.basename(data_abs_path)
                labelme_json_instance = configs.gen_labelme_json_model(img_size, img_name)
                weakness_list = xml_label_dict['outputs']['object']['item']
                weakness_list = weakness_list if isinstance(weakness_list, list) else [weakness_list]
                for weakness_item in weakness_list:
                    label_kind = label_kind_to_opt.keys() & weakness_item.keys()
                    assert len(label_kind) == 1, 'ERROR: Fail to read the {} in {}.'.format(str(weakness_item),
                                                                                            xml_file_path)
                    label_kind = label_kind.pop()
                    self.update_stat_dict(weakness_item.get('name'))
                    shapes_result_item, warning_list = label_kind_to_opt[label_kind](weakness_item)
                    if warning_list:
                        print('WARNING: {} occured while processing {}.'.format(xml_file_path, str(warning_list)))
                    labelme_json_instance['shapes'].append(shapes_result_item)
                with open(labelme_json_out_file, 'w', encoding='utf-8') as out_file:
                    json.dump(labelme_json_instance, out_file, ensure_ascii=False, indent=2)
                if not self.disable_jpg:
                    img_file = read_complete_image(data_abs_path)
                    cv2.imwrite(os.path.join(self.output_path, data_id + '.jpg'), img_file,
                ...
                fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
                print(exc_type, fname, exc_tb.tb_lineno)
                print("Fail to convert the data {}, ERROR: {}.".format(xml_file_path, str(e)))
    ...
    def json_to_labelme(self, img_dict: dict, data_id: str, json_item):
        if img_dict.get(data_id):
            try:
                ...
                assert len(label_json.keys() & configs.gen_labelme_json_model(None,
                                                                              None).keys()) >= 4, 'The format of json file {} is not correct.'.format(
                    json_item)
                ...
                label_json['imagePath'] = data_id + '.jpg' if not self.disable_jpg else os.path.basename(
                    img_dict[data_id])
                with open(os.path.join(self.output_path, data_id + '.json'), 'w', encoding='utf-8') as out_file:
                ...
                print("Fail to convert the data {}, ERROR: {}.".format(json_item, str(e)))
                if os.path.exists(os.path.join(self.output_path, data_id + '.jpg')):
                    os.remove(os.path.join(self.output_path, data_id + '.jpg'))
                if os.path.exists(os.path.join(self.output_path, data_id + '.json')):
                    os.remove(os.path.join(self.output_path, data_id + '.json'))
    ...
    def collect_excel_detail(self, map_config, excel_file) -> dict:
        try:
            ...
            for label_item in label_list:
                if len(label_item['task_id']) <= 4 and len(label_item['product_sn']) <= 4 and len(
                        label_item['picture_id']) <= 2:
                    data_id = '{}-{}-{}'.format(label_item['task_id'].rjust(4, '0'),
                                                label_item['product_sn'].rjust(4, '0'),
                                                label_item['picture_id'].rjust(2, '0'))
                    print(data_id)
                    self.update_stat_dict(label_item.get('label'))
                    label_shape_item = {'label': pypinyin.slug(label_item.pop('label'), separator=''),
                                        'shape_type': 'rectangle',
                                        'points': [[label_item['x_min'], label_item['y_min']],
            ...
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            print(exc_type, fname, exc_tb.tb_lineno)
            print('Failed to collect info from {}. WARNING: {}'.format(excel_file, str(e)))
            return {}
    ...
    def convert_tianchi_geometry(geometry_list):
        ...
        return [point_1, point_2] if geometry_list[0] < geometry_list[2] else [point_2, point_1]
    ...
    def collect_csv_detail(self, csv_file) -> dict:
        try:
            ...
            for label_item in label_list:
                if label_item[4] and label_item[5]:
                    ...
                    for shape_item in label_json.get('items'):
                        ...
                        label_shape_item = {'label': shape_item.get('labels').get('标签'),
                                            'shape_type': 'rectangle',
                                            'points': GenLabelme.convert_tianchi_geometry(shape_item.get('meta').get('geometry'))}
                        if labelme_shape_dict.get(data_id):
                            labelme_shape_dict[data_id].append(label_shape_item)
                        else:
                            labelme_shape_dict[data_id] = [label_shape_item]
            ...
            print('Failed to collect info from {}. WARNING: {}'.format(csv_file, str(e)))
            return {}

    # source_data_path consisted of images
    def assemble_labelme(self, source_data_path: str, data_id: str, shape_item):
        try:
            img_file_path = image_filter(glob.glob(os.path.join(source_data_path, data_id + '*')))
            if len(img_file_path) > 1:
                raise Exception('Multiple image files detected for ID: {}.'.format(data_id))
                ...
                img_size = {'width': img_file.shape[1], 'height': img_file.shape[0], 'depth': img_file.shape[2]}
                img_name = data_id + '.jpg' if not self.disable_jpg else os.path.basename(img_file_path)
            ...
            print("Fail to convert the data {}, WARNING: {}.".format(self.excel_file, str(e)))
    ...
    def trigger_generate(self):
        if self.excel_file:
            ...
            work_dir_list = glob.glob(os.path.join(self.work_dir, '*')) if self.recursive else [self.work_dir]
        ...
        if data_list.get('xml_list'):
            ...
            for xml_source_path in tqdm(data_list['xml_list'], desc='xml_list'):
                ...
                for img_data_item in img_data_list:
                    xml_pool.apply_async(self.xml_to_labelme,
        ...
        if data_list.get('excel_list'):
            ...
            for excel_path in tqdm(data_list['excel_list']):
                labelme_shape_dict = self.collect_excel_detail(self.map_config, self.excel_file)
                if labelme_shape_dict:
                    process_bar = tqdm(total=len(labelme_shape_dict))
                    for data_id, shape_item in labelme_shape_dict.items():
                        excel_pool.apply_async(self.assemble_labelme,
            ...
            for json_path in tqdm(data_list['json_list'], desc='json_list'):
                ...
                if not json_dict or not img_dict:
                    ...
                    for data_id, json_item in json_dict.items():
                        json_pool.apply_async(self.json_to_labelme,
        ...
        if data_list.get('csv_list'):
            ...
            for csv_path in tqdm(data_list['csv_list']):
                ...
                if labelme_shape_dict:
                    process_bar = tqdm(total=len(labelme_shape_dict))
                    for data_id, shape_item in labelme_shape_dict.items():
                        csv_pool.apply_async(self.assemble_labelme,
...
def get_parser():
    ...
    parser.add_argument('-w', '--work_dir', required=True, type=str, default=None, help='The path of the work_dir.')
    ...
    parser.add_argument('--disable_jpg', action='store_true',
                        help='Whether disable the function of converting origin image to the JPEG format. (default: False)')
    parser.add_argument('-x', '--excel_file', type=str, default=None,
                        help='The label file stored with excel format. (Required if convert excel format label)')
    parser.add_argument('--csv_file', type=str, default=None, help='The label file stored with csv format. (For the Tianchi challenge)')
    parser.add_argument('-c', '--map_config', type=str, default=None,
    ...
    parser.add_argument('-p', '--process_num', type=int, default=10,
                        help='The num of workers for multiprocess. (default: 8)')
    opt = parser.parse_args()

    if not os.path.isdir(opt.work_dir):
        print('ERROR: The work_dir given ({}) is not a valid path.'.format(opt.work_dir))
        sys.exit()
    if opt.excel_file:
        if not opt.output_path:
            print('ERROR: The output_path is required while converting.'.format(opt.output_path))
            sys.exit()
        if not opt.map_config:
            print('ERROR: The map_config is required while converting.'.format(opt.excel_file))
            sys.exit()
    opt.output_path = opt.output_path if opt.output_path else os.path.abspath(
        os.path.join(opt.work_dir, '..'))
    return opt
