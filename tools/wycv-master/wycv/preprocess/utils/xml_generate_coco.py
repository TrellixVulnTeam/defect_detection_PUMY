...
class Xml2Labelme(object):
    def __init__(self, annotation_root_path, jsons_path, result_write, process_num=8):
        self.xml_outputs_path = os.path.join(annotation_root_path, 'outputs-rending')
    ...
    def transform_xml_2labelme(self, xml_path, save_json, image_name, class_py_ch_dic):
        ...
        try:
            ...
            print('except xml file :{}'.format(xml_path))
        dic_all = {}
        dic_all['version'] = '1.0'
        dic_all['flags'] = {}
...
class CutLabelme(object):
    def __init__(self, annotation_root_path, img_jsons, out_path_result_cut, cut_label, process_nums, cut_w, cut_h,
                 result_write, expect_annotations_file):
    ...
    def cut_json(self, json_p):
        ...
        for index_object in range(len(cut_one_img)):
            ...
            new_name_img = '{}_{}_{}.jpg'.format(mid_point[index_object][0], mid_point[index_object][1],
                                                 index_object)
            new_name_json = '{}_{}_{}.json'.format(mid_point[index_object][0], mid_point[index_object][1],
                                                   index_object)
        ...
        if not os.path.exists(self.expect_annotations_file):
            ...
            for img_name in source_img_dic:
                if not img_name in cut_img_dic:
                    ...
                    shutil.copy(os.path.join(self.source_jsons_path, jsons_name),
                                os.path.join(self.expect_annotations_file, jsons_name))
...
class Select_data(object):
    def __init__(self, data_rootpath, save_path):
        xml_path = os.path.join(data_rootpath, 'outputs-rending')
...
class labelme2coco(object):
    def __init__(self, labelme_json=[], save_json_path='./new.json', valid_labels=[], resume_cate=None):
        '''
        ...
        for num, json_file in enumerate(self.labelme_json):
            with open(json_file, 'r', encoding='utf-8') as fp:
                ...
                for shapes in data['shapes']:
                    ...
                    if label[0] not in self.valid_labels:
                        continue
    ...
    def save_json(self):
        ...
        # 保存json文件
        json.dump(self.data_coco, open(self.save_json_path, 'w', encoding='utf-8'), indent=4)


if __name__ == "__main__":
    annotation_root_path = r'D:\data\module-c\remove-tiny\damian'
    out_coco_root_path = r'D:\data\module-c\remove-tiny\damian'
    # Select_data(annotation_root_path, out_coco_root_path)  # 过滤xml与img不符合情况
    #
    # img_jsons = os.path.join(annotation_root_path, 'jsons')
    # process_nums = 8
    # result_write = os.path.join(out_coco_root_path, 'class_label.txt')
    # xml2json = Xml2Labelme(annotation_root_path, img_jsons, result_write, process_nums)
    # #
    # cut_label = xml2json.cut_label
    # cut_w, cut_h = 2048, 2048
    ...
    # #
    # print(annotation_root_path,img_jsons,out_path_result_cut,cut_label,process_nums,cut_w,cut_h,result_write,expect_annotations_file)
    # cut_labelme = CutLabelme(annotation_root_path, img_jsons, out_path_result_cut, cut_label, process_nums, cut_w,
    #                          cut_h, result_write,expect_annotations_file)
    ...
    # label_converter(labelme_json, coco_path)

    # annotation_root_path = r'F:\module-c\c-daowen\daowen'
    # img_jsons = r'F:\module-c\c-daowen\daowen\jsons'
    # out_path_result_cut = r'F:\module-c\c-daowen\daowen\cuts'
    # cut_label = ('shahenyin', 'tabian', 'daowen', 'mianhua', 'aotuhen')
    # process_nums = 8
    # cut_w = 1024
    # cut_h = 1024
    # result_write = r'F:\module-c\c-daowen\daowen\class_label.txt'
    # expect_annotations_file = r'F:\module-c\c-daowen\daowen\daowen\except'
    # cut_labelme = CutLabelme(annotation_root_path, img_jsons, out_path_result_cut, cut_label, process_nums, cut_w,
    #                          cut_h, result_write,expect_annotations_file)

    labelme_json = glob.glob('{}/*.json'.format(r'F:\c-only-rending\train'))
    coco_path = os.path.join(r'F:\c-only-rending', 'train.json')
    valid_labels = ['heidian', 'pengshang', 'guashang', 'shahenyin', 'baidian', 'tabian', 'daowen', 'yiwu', 'aokeng',
                    'mianhua', 'yise2', 'yise1', 'aotuhen']
    labelme2coco(labelme_json, coco_path, valid_labels)
