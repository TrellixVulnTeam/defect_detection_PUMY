...
def convert_annotation(image_id):
    ...
    in_file = open('aug/annotations/%s.xml' % (image_id), encoding='utf-8')
    # 准备在对应的image_id 中写入对应的label，分别为
    # <object-class> <x> <y> <width> <height>
    out_file = open('aug/labels/%s.txt' % (image_id), 'w', encoding='utf-8')
    # 解析xml文件
...
　　　　２．同时对所有的图片文件进行解析和转化，将其对应的bundingbox 以及类别的信息全部解析写到label 文件中去
    ...
    '''
    # 先找labels文件夹如果不存在则创建
    if not os.path.exists('aug/labels/'):
        os.makedirs('aug/labels/')
    # 读取在ImageSets/Main 中的train、test..等文件的内容
    # 包含对应的文件名称
    image_ids = open('aug/ImageSets/%s.txt' % (image_set)).read().strip().split()
    # 打开对应的2012_train.txt 文件对其进行写入准备
    list_file = open('aug/%s.txt' % (image_set), 'w')
