...
if __name__ == '__main__':
    ...
    　　　　２．同时对所有的图片文件进行解析和转化，将其对应的bundingbox 以及类别的信息全部解析写到label 文件中去
        ...
        if not os.path.exists(os.path.join(work_dir, 'labels')):
            os.makedirs(os.path.join(work_dir, 'labels'))
        # f_ = open(os.path.join(work_dir,'ImageSets','%s.txt'%image_set), 'r')
        # 读取在ImageSets/Main 中的train、test..等文件的内容
        # 包含对应的文件名称os.path.join(ori,'ImageSets/%s.txt' % (image_set))

        image_ids = open(os.path.join(work_dir, 'ImageSets/%s.txt' % image_set)).read().strip().split()
        # 打开对应的2012_train.txt 文件对其进行写入准备
        list_file = open(os.path.join(work_dir, '%s.txt' % image_set), 'w')
        # 将对应的文件_id以及全路径写进去并换行
        for image_id in image_ids:
            list_file.write(os.path.join(work_dir, 'images/%s.jpg' % image_id))
            # 调用  year = 年份  image_id = 对应的文件名_id
            convert_annotation(work_dir, image_id)
        # 关闭文件
        list_file.close()
        f_ = open(os.path.join(work_dir, 'ImageSets', '%s.txt' % image_set), 'r')
        save_f = open(os.path.join(work_dir, '%s.txt' % image_set), 'w+')
        line = f_.readlines()
        for line_ in line:
            line_ = line_.replace('\n', '')
            save_f.write(os.path.join(work_dir, 'images', '%s.jpg\n' % line_))
        f_.close()
        save_f.close()
