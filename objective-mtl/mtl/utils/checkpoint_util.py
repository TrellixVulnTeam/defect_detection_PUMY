...
def get_external_models(default_json_path, external_json_path):
    default_urls = file_load(default_json_path)
    assert isinstance(default_urls, dict)
    if osp.exists(external_json_path):
        external_urls = file_load(external_json_path)
...
def load_checkpoint(model, filename, map_location=None, strict=False, logger=None):
    ...
    #     if k.startswith('module.encoder_q') and not k.startswith('module.encoder_q.fc') and not k.endswith('num_batches_tracked'):
