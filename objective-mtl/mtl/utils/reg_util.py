...
def build_module_from_dict(cfg_dict, registry, default_args=None):
    ...
    if "type" not in cfg_dict:
        print("workspace/data/obj-mtl/mtl/utils/reg_util.py~~~~~")
        print(cfg_dict)
    ...
    type_value = cfg_dict.pop("type")
    if default_args is not None and len(default_args) > 0:
        res_model = module_cls(**cfg_dict, **default_args)
    else:
        res_model = module_cls(**cfg_dict)
    cfg_dict["type"] = type_value
    return res_model
