...
def resize_img_map(img, sd_map):
    ...
    f_sd_map = impad(sd_map, padding=paddings, pad_val=0)
...
def get_regions(patch_scores, patch_size, patch_num=10, ratio=(4, 4)):
    ...
    for j in range(patch_num - ratio[0] + 1):
        for i in range(patch_num - ratio[1] + 1):
            ...
            rg_score = (
                rg_score / (ratio[0] * ratio[1]) * (math.sqrt(ratio[0] + ratio[1]))
            )
...
def topk_regions(rg_props, k):
    sorted_rgs = sorted(rg_props, key=lambda x: x["rg_score"], reverse=True)
    return sorted_rgs[:k]
...
def get_proposals_from_saliency(
    img,
    mask_map=None,
    img_normalizer=None,
    ...
    sd_map = get_saliency_map(img, saliency_type, img_normalizer, fixed_size, is_resize)
    ...
    for raw_ratio in partition_ratios:
        ...
        for ratio in extended_ratios:
            rg_props.extend(get_regions(patch_scores, patch_size, patch_num, ratio))
            ...
            if key_node_handle is not None:
                for i in range(len(key_node_names)):
                    if key_node_handle.find(key_node_names[i]) is not None:
                        key_nodes[key_node_names[i]] = list(
                            eval(key_node_handle.find(key_node_names[i]).text)
                        )
        ...
        for face_item in anno_data["human_face"]["data"]["resultList"][0]["faceList"]:
            ...
            try:
                face_node_list.append(
                    {
                        "lefteye": [
                            float(face_item["lm"]["points"][0]["x"]),
                            float(face_item["lm"]["points"][0]["y"]),
                        ],
                        "righteye": [
                            float(face_item["lm"]["points"][1]["x"]),
                            float(face_item["lm"]["points"][1]["y"]),
                        ],
                        "nose": [
                            float(face_item["lm"]["points"][2]["x"]),
