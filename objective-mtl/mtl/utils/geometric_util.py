...
from .io_util import imread_backend
...
def imcrop_center(img, size, scale=1.0, pad_fill=None):
    """Center crop image.
    Args:
    ...
    pad_w = (w - size[0]) // 2
    pad_h = (h - size[1]) // 2
    bboxes = np.array([pad_w, pad_h, pad_w + size[0] - 1, pad_h + size[1] - 1])
    return imcrop(img, bboxes, scale=scale, pad_fill=pad_fill)
