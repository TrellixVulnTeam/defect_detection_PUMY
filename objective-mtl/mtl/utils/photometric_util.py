...
def tensor2imgs(tensor, mean=(0, 0, 0), std=(1, 1, 1), to_rgb=True):
    ...
    for img_ind in range(num_imgs):
        img = tensor[img_ind, ...].cpu().numpy().transpose(1, 2, 0)
...
def adjust_sharpness(img, factor=1.0, kernel=None):
    """Adjust image sharpness. This function controls the sharpness of an
    image. An enhancement factor of 0.0 gives a blurred image. A factor of 1.0
    gives the original image. And a factor of 2.0 gives a sharpened image. It
    blends the source image and the degenerated mean image:
