...
class InceptionRegNet(nn.Module):
    ...
    def __init__(
        ...
        cfg_dict = {"conv_cfg": conv_cfg, "norm_cfg": norm_cfg, "act_cfg": act_cfg}
        ...
        self.conv_2_1 = ConvModule(
            in_channels[0], branch_out_dim, 3, padding=1, **cfg_dict
        )
        self.pool_2_2 = nn.AdaptiveMaxPool1d(output_size=1)

        self.conv_3_1 = ConvModule(
            in_channels[0], branch_out_dim, 3, padding=1, **cfg_dict
        )
        self.conv_3_2 = ConvModule(
            branch_out_dim, branch_out_dim, 3, padding=1, **cfg_dict
        )
        self.pool_3_3 = nn.AdaptiveMaxPool1d(output_size=1)

        self.conv_4_1 = ConvModule(
            in_channels[0], branch_out_dim, 3, padding=1, **cfg_dict
        )
        self.conv_4_2 = ConvModule(
            branch_out_dim, branch_out_dim, 3, padding=1, **cfg_dict
        )
        self.conv_4_3 = ConvModule(
            branch_out_dim, branch_out_dim, 3, padding=1, **cfg_dict
        )
        ...
        self.conv_6_1 = ConvModule(
            in_channels[1], branch_out_dim, 3, padding=1, **cfg_dict
        )
        self.pool_6_2 = nn.AdaptiveMaxPool1d(output_size=1)

        self.conv_7_1 = ConvModule(
            in_channels[1], branch_out_dim, 3, padding=1, **cfg_dict
        )
        self.conv_7_2 = ConvModule(
            branch_out_dim, branch_out_dim, 3, padding=1, **cfg_dict
        )
        self.pool_7_3 = nn.AdaptiveMaxPool1d(output_size=1)

        self.conv_8_1 = ConvModule(
            in_channels[1], branch_out_dim, 3, padding=1, **cfg_dict
        )
        self.conv_8_2 = ConvModule(
            branch_out_dim, branch_out_dim, 3, padding=1, **cfg_dict
        )
        self.conv_8_3 = ConvModule(
            branch_out_dim, branch_out_dim, 3, padding=1, **cfg_dict
        )
