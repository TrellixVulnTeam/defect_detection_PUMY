...
from mtl.cores.layer_ops import yolo_layer as yolox_layer
...
class CSPDarknet(nn.Module):
    def __init__(
        ...
        Conv = yolox_layer.DWConv if depthwise else yolox_layer.BaseConv

        base_channels = int(wid_mul * 64)  # 64
        base_depth = max(round(dep_mul * 3), 1)  # 3

        # stem
        self.stem = yolox_layer.Focus(3, base_channels, ksize=3, act=act)

        # dark2
        self.dark2 = nn.Sequential(
            Conv(base_channels, base_channels * 2, 3, 2, act=act),
            yolox_layer.CSPLayer(
                base_channels * 2,
                base_channels * 2,
                n=base_depth,
                depthwise=depthwise,
        ...
        self.dark5 = nn.Sequential(
            ...
            yolox_layer.SPPBottleneck(base_channels * 16, base_channels * 16, activation=act),
            yolox_layer.CSPLayer(
                base_channels * 16,
                base_channels * 16,
                n=base_depth,
