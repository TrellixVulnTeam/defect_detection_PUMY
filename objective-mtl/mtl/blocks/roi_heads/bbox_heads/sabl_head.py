...
class SABLHead(nn.Module):
    ...
    def init_weights(self):
        for module_list in [self.reg_cls_fcs, self.reg_offset_fcs, self.cls_fcs]:
    ...
    def _attention_pool(self, reg_x):
    ...
    def _reg_pred(self, x, offfset_fcs, cls_fcs):
    ...
    def _side_aware_split(self, feat):
    ...
    def _reg_forward(self, reg_x):
        outs = self._side_aware_feature_extractor(reg_x)
        edge_offset_preds = []
        edge_cls_preds = []
        reg_fx = outs[0]
        reg_fy = outs[1]
        offset_pred_x, cls_pred_x = self._reg_pred(
            reg_fx, self.reg_offset_fcs, self.reg_cls_fcs
        )
        offset_pred_y, cls_pred_y = self._reg_pred(
            reg_fy, self.reg_offset_fcs, self.reg_cls_fcs
        )
        offset_pred_x = self._side_aware_split(offset_pred_x)
        offset_pred_y = self._side_aware_split(offset_pred_y)
        cls_pred_x = self._side_aware_split(cls_pred_x)
        cls_pred_y = self._side_aware_split(cls_pred_y)
    ...
    def forward(self, x):
        bbox_pred = self._reg_forward(x)
        cls_score = self._cls_forward(x)
    ...
    def get_targets(self, sampling_results, gt_bboxes, gt_labels, rcnn_train_cfg):
        ...
        cls_reg_targets = self._bucket_target(
            pos_proposals, neg_proposals, pos_gt_bboxes, pos_gt_labels, rcnn_train_cfg
        )
        (
            labels,
            label_weights,
    ...
    def _bucket_target_single(
        self, pos_proposals, neg_proposals, pos_gt_bboxes, pos_gt_labels, cfg
    ):
    ...
    def get_bboxes(
        ...
        if bbox_pred is not None:
            bboxes, confids = self.bbox_coder.decode(rois[:, 1:], bbox_pred, img_shape)
        else:
            bboxes = rois[:, 1:].clone()
            confids = None
