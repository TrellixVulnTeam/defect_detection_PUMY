...
class MaskPointHead(nn.Module):
    ...
    def _get_target_single(
        self, rois, rel_roi_points, pos_assigned_gt_inds, gt_masks, cfg
    ):
    ...
    def get_losses(self, point_pred, point_targets, labels):
        ...
        if self.class_agnostic:
            loss_point = self.loss_point(
                point_pred, point_targets, torch.zeros_like(labels)
            )
        else:
    ...
    def get_roi_rel_points_test(self, mask_pred, pred_label, cfg):
        ...
        point_coords[:, :, 0] = (
            w_step / 2.0 + (point_indices % mask_width).float() * w_step
        )
        point_coords[:, :, 1] = (
            h_step / 2.0 + (point_indices // mask_width).float() * h_step
        )
