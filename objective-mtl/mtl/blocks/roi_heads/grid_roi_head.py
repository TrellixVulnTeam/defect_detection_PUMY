...
class GridRoIHead(StandardRoIHead):
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels, img_metas):
        ...
        bbox_results = super(GridRoIHead, self)._bbox_forward_train(
        ...
        loss_grid = self.grid_head.get_losses(grid_pred, grid_targets)
            ...
            for i in range(num_imgs):
                if det_bboxes[i].shape[0] == 0:
                    ...
                    if torch.onnx.is_in_onnx_export():
                        bbox_results = det_bbox
                    else:
                        bbox_results.append(
                            bbox2result(
                                det_bbox, det_labels[i], self.bbox_head.num_classes
                            )
                        )
        else:
            bbox_results = [grid_rois.new_tensor([]) for _ in range(len(det_bboxes))]
