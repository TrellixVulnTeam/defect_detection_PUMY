...
from .roi_test_mixins import BBoxTestMixin, MaskTestMixin
from mtl.cores.bbox import bbox2result, bbox2roi, bbox_mapping
...
class CascadeRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def init_bbox_head(self, bbox_roi_extractor, bbox_head):
        ...
        if not isinstance(bbox_roi_extractor, list):
            bbox_roi_extractor = [
                copy.deepcopy(bbox_roi_extractor) for _ in range(self.num_stages)
            ]
        if not isinstance(bbox_head, list):
            bbox_head = [copy.deepcopy(bbox_head) for _ in range(self.num_stages)]
    ...
    def _bbox_forward_train(
        ...
        loss_bbox = self.bbox_head[stage].get_losses(
    ...
    def _mask_forward_train(
        ...
        loss_mask = self.mask_head[stage].get_losses(
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            bbox_results = [
                bbox2result(
                    det_bboxes[i], det_labels[i], self.bbox_head[-1].num_classes
                )
                for i in range(num_imgs)
            ]
        ...
        if self.with_mask:
            if torch.onnx.is_in_onnx_export():
                results = (
                    ms_bbox_result["ensemble"][0],
                    ms_bbox_result["ensemble"][1],
