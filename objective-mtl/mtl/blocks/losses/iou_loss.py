    ...
    def forward(
        ...
        **kwargs,
        ...
        if True in torch.isnan(loss):
            ...
            logger.info(f"Relevant IoULoss Variables: {pred.sum()}, {target.sum()}")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if torch.isnan(loss):
            logger = get_root_logger()
            logger.info(
                f"Relevant BoundedIoULoss Variables: {pred.sum()}, {target.sum()}"
            )
            # raise ValueError("loss is nan.")
            return torch.Tensor([0]).to(pred.device).sum()
        else:
...
class DIoULoss(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if torch.isnan(loss):
            logger = get_root_logger()
            logger.info(f"Relevant DIoULoss Variables: {pred.sum()}, {target.sum()}")
            # raise ValueError("loss is nan.")
            return torch.Tensor([0]).to(pred.device).sum()
        else:
            return self.loss_weight * loss
...
class CIoULoss(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if torch.isnan(loss):
            logger = get_root_logger()
            logger.info(f"Relevant CIoULoss Variables: {pred.sum()}, {target.sum()}")
            # raise ValueError("loss is nan.")
            return torch.Tensor([0]).to(pred.device).sum()
        else:
            return self.loss_weight * loss
