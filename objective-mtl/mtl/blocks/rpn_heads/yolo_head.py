...
class YOLOV3Head(BaseDetHead, BBoxTestMixin):
    ...
    def get_bboxes(self, pred_maps, img_metas, cfg=None, rescale=False, with_nms=True):
        ...
        for _idx in range(len(img_metas)):
            pred_maps_list = [pred_maps[i][_idx].detach() for i in range(num_levels)]
    ...
    def get_losses(
        ...
        for _idx in range(len(img_metas)):
            responsible_flag_list.append(
                self.anchor_generator.responsible_flags(
                    featmap_sizes, gt_bboxes[_idx], device
                )
            )
        ...
        losses_cls, losses_conf, losses_xy, losses_wh = multi_apply(
            self._get_loss_single, pred_maps, target_maps_list, neg_maps_list
        )
    ...
    def _get_loss_single(self, pred_map, target_map, neg_map):
