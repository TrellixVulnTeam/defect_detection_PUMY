...
class AnchorHead(BaseDetHead, BBoxTestMixin):
        ...
        if self.train_cfg:
            if isinstance(self.train_cfg, CfgNode):
                ...
                if self.sampling and ("sampler" in self.train_cfg):
                    sampler_cfg = self.train_cfg["sampler"].copy()
                else:
                    sampler_cfg = dict(type="PseudoSampler")
        ...
        Returns:
            tuple: A tuple of classification scores and bbox prediction.
                ...
                - bbox_preds (list[Tensor]): Box energies / deltas for all \
                    ...
                    is num_anchors * 4.
        """
        return multi_apply(self._forward_single, feats)
    ...
    def _get_loss_single(
        ...
        if num_total_samples > 0:
            ...
            if math.isnan(loss_bbox.mean()):
                print("Error bbox loss: ", bbox_pred, bbox_targets, bbox_weights)
        else:
    ...
    def get_bboxes(
        ...
        Example:
            ...
            >>> cfg = Config(dict(
            >>>     score_thr=0.00,
            >>>     nms=dict(type='common_nms', iou_threshold=1.0),
            >>>     max_per_img=10))
            >>> feat = torch.rand(1, 1, 3, 3)
            >>> cls_score, bbox_pred = self._forward_single(feat)
