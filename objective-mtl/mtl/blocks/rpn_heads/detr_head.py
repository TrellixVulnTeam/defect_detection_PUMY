...
from mtl.cores.ops.ops_builder import build_activation_layer
from mtl.cores.layer_ops.transformer_layer import FFN
...
from mtl.utils.misc_util import multi_apply, dynamic_clip_for_onnx
...
class DETRHead(AnchorFreeHead):
    ...
    Args:
        ...
        loss_cls (dict): Config of the
            classification loss. Default `CrossEntropyLoss`.
        loss_bbox (dict): Config of the
            regression loss. Default `L1Loss`.
        loss_iou (dict): Config of the
            regression iou loss. Default `GIoULoss`.
        tran_cfg (dict): Training config of
            transformer head.
        test_cfg (dict): Testing config of
            transformer head.
    ...
    def forward(self, x):
        ...
        """
        all_cls_scores = self.fc_cls(x)
        all_bbox_preds = self.fc_reg(self.activate(self.reg_ffn(x))).sigmoid()
        ...
        Returns:
            tuple[list[Tensor], list[Tensor]]: Outputs for all scale levels.
                ...
                - all_bbox_preds_list (list[Tensor]): Sigmoid regression \
                    ...
                    [nb_dec, bs, num_query, 4].
        """
        return multi_apply(self._forward_single_onnx, feats)

    def _forward_single_onnx(self, x):
