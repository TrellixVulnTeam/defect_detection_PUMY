...
from mtl.cores.ops import ConvModule, Scale
...
class ATSSHead(AnchorHead):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.cls_stacked_convs = cls_stacked_convs
        self.reg_stacked_convs = reg_stacked_convs
        self.conv_cfg = conv_cfg
        self.norm_cfg = norm_cfg
        super(ATSSHead, self).__init__(
            num_classes, in_channels, reg_decoded_bbox=reg_decoded_bbox, **kwargs
        )
        ...
        if self.train_cfg:
            if isinstance(self.train_cfg, CfgNode):
                ...
                assigner_dict = self.train_cfg["assigner"].copy()
                self.assigner = build_assigner(assigner_dict)
    ...
    def _init_layers(self):
        ...
        for i in range(self.reg_stacked_convs):
            chn = self.in_channels if i == 0 else self.feat_channels
        ...
        Returns:
            tuple: Usually a tuple of classification scores and bbox prediction
                ...
                bbox_preds (list[Tensor]): Box energies / deltas for all scale
                    ...
                    num_anchors * 4.
        """
        return multi_apply(self._forward_single, feats, self.scales)
    ...
    def _forward_single(self, x, scale):
        ...
        Args:
            ...
            scale (:obj: `Scale`): Learnable scale module to resize
                the bbox prediction.
    ...
    def _get_loss_single(
        ...
        if len(pos_inds) > 0:
            ...
            loss_centerness = self.loss_centerness(
                pos_centerness, centerness_targets, avg_factor=num_total_samples
            )
    ...
    def get_bboxes(
        ...
        for _idx in range(len(img_metas)):
            cls_score_list = [cls_scores[i][_idx].detach() for i in range(num_levels)]
            bbox_pred_list = [bbox_preds[i][_idx].detach() for i in range(num_levels)]
            centerness_pred_list = [
                centernesses[i][_idx].detach() for i in range(num_levels)
            ]
            img_shape = img_metas[_idx]["img_shape"]
