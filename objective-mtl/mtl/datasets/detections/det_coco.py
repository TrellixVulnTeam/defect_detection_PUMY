...
class CocoDataset(DetBaseDataset):
    ...
    def get_ann_info(self, idx):
        ...
        img_ind = self.data_infos[idx]["id"]
        ann_ids = self.coco.getAnnIds(imgIds=[img_ind])
        ann_info = self.coco.loadAnns(ann_ids)
        return self._parse_ann_info(self.data_infos[idx], ann_info)

    def getitem_info(self, index, return_img=True):
        if self.is_tfrecord:
            return super(CocoDataset, self).getitem_info(index, return_img)
        else:
            if not return_img:
                ...
                self.data_infos[index]["ann"] = self.get_ann_info(index)
    ...
    def get_cat_ids(self, idx):
        ...
        """
        img_ind = self.data_infos[idx]["file_name"]
        ann_ids = self.coco.getAnnIds(imgIds=[img_ind])
        ann_info = self.coco.loadAnns(ann_ids)
    ...
    def _filter_imgs(self, min_size=32):
        ...
        for i, img_info in enumerate(self.data_infos):
            img_ind = self.img_ids[i]
            if self.filter_empty_gt and img_ind not in ids_in_cat:
                continue
            if min(img_info["width"], img_info["height"]) >= min_size:
                valid_inds.append(i)
                valid_img_ids.append(img_ind)
    ...
    def _proposal2json(self, results):
        ...
        for idx in range(len(self)):
            img_ind = self.img_ids[idx]
            bboxes = results[idx]
            for i in range(bboxes.shape[0]):
                data = dict()
                data["image_id"] = img_ind
