...
from mtl.utils.io_util import check_integrity, download_and_extract_archive
from ..data_builder import DATASETS
from .cls_base import ClsBaseDataset


@DATASETS.register_module()
class CIFAR10(ClsBaseDataset):
    ...
    def load_annotations(self):
        if not self._check_integrity():
            download_and_extract_archive(
                self.url, self.data_root, filename=self.filename, md5=self.tgz_md5
            )

        if self.val_mode or self.test_mode:
        ...
        for file_name, _ in downloaded_list:
            file_path = os.path.join(self.data_prefix, file_name)
        ...
        for img, gt_label in zip(self.imgs, self.gt_labels):
            ...
            info = {"img": img, "gt_label": gt_label, "width": 32, "height": 32}
    ...
    def _load_meta(self):
        path = os.path.join(self.data_prefix, self.meta["filename"])
