...
class ConcatDataset(_ConcatDataset):
    ...
    def __init__(self, datasets, separate_eval=True):
        """Initialization for concat datasets
        Args:
            datasets (list[:obj:`Dataset`]): A list of datasets.
            separate_eval (bool, optional): [description]. Defaults to True.

        Raises:
            NotImplementedError: make sure that all datasets have the same type
        """
        super(ConcatDataset, self).__init__(datasets)
        if hasattr(datasets[0], "class_names"):
            self.class_names = datasets[0].class_names
        self.separate_eval = separate_eval
        if not separate_eval:
            if len(set([type(ds) for ds in datasets])) != 1:
                raise NotImplementedError("All the datasets should have the same type")
    ...
    def evaluate(self, results, logger=None, **kwargs):
        ...
        if self.separate_eval:
            ...
            if hasattr(self.datasets[0], data_infos):
