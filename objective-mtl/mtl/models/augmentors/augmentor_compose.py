...
from ..model_builder import build_augmentor


class AugmentConstructor(object):
    """Data augments.
    We implement some data augmentation methods, such as mixup, cutmix.

    Args:
        augments_cfg: Config dict of augments
    ...
    def __init__(self, augments_cfg):
        super(AugmentConstructor, self).__init__()

        if isinstance(augments_cfg, dict):
            augments_cfg = [augments_cfg]

        assert len(augments_cfg) > 0, "The length of augments_cfg should be positive."
        has_identity = any([cfg["type"] == "Identity" for cfg in augments_cfg])
        self.augments = [build_augmentor(cfg) for cfg in augments_cfg]
        ...
        if has_identity:
            ...
            if identity_prob > 0:
                ...
                self.augments += [
                    build_augmentor(
                        dict(
                            type="Identity", num_classes=num_classes, prob=identity_prob
                        )
                    )
                ]
    ...
    def __call__(self, img, gt_label):
        if self.augments:
            if gt_label.dim() == 2 and gt_label.shape[1] == 1:
