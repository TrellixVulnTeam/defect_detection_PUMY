...
def bbox2roi(bbox_list):
    ...
    """
    rois_list = []
    for img_ind, bboxes in enumerate(bbox_list):
        if bboxes.size(0) > 0:
            img_inds = bboxes.new_full((bboxes.size(0), 1), img_ind)
...
def roi2bbox(rois):
    ...
    """
    bbox_list = []
    img_inds = torch.unique(rois[:, 0].cpu(), sorted=True)
    for img_ind in img_inds:
        inds = rois[:, 0] == img_ind.item()
        bbox = rois[inds, 1:]
