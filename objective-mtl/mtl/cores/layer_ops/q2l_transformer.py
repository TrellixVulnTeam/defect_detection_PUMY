...
from .transformer_layer import _get_activation_fn
from .transformer_ops import _get_clones


class Q2LTransformerEncoderLayer(nn.Module):
    def __init__(
        self,
        d_model,
        nhead,
...
class Q2LTransformerDecoderLayer(nn.Module):
    def __init__(
        self,
        d_model,
        nhead,
...
class Q2LTransformerEncoder(nn.Module):
...
class Q2LTransformerDecoder(nn.Module):
...
class Q2LTransformer(nn.Module):
    """
    ...
    def __init__(
        ...
        if num_decoder_layers > 0:
            encoder_layer = Q2LTransformerEncoderLayer(
                d_model, nhead, dim_feedforward, dropout, activation, normalize_before
            )
            encoder_norm = nn.LayerNorm(d_model) if normalize_before else None
            self.encoder = Q2LTransformerEncoder(
                encoder_layer, num_encoder_layers, encoder_norm
            )

        decoder_layer = Q2LTransformerDecoderLayer(
            d_model, nhead, dim_feedforward, dropout, activation, normalize_before
        )
        decoder_norm = nn.LayerNorm(d_model)
        self.decoder = Q2LTransformerDecoder(
            decoder_layer,
            num_decoder_layers,
            decoder_norm,
