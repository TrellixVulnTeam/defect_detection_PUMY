...
class YOLOV4AnchorGenerator(YOLOAnchorGenerator):
    ...
    def responsible_indices(
        self,
        featmap_sizes,
        gt_bboxes_list,
        neighbor=3,
        shape_match_thres=4.0,
        device="cuda",
    ):
        ...
        for ind, gt_bboxes in enumerate(gt_bboxes_list):
            ...
            img_all_id.append(gt_bboxes.new_full((num_gt,), ind, dtype=torch.long))

        gt_bboxes = torch.cat(gt_bboxes_list, dim=0)
        img_all_id = torch.cat(img_all_id, dim=0)
        ...
        neighbor_offset = gt_xy.new_tensor(
            [
                [0, 0],  # current grid
        ...
        for i in range(self.num_levels):
            ...
            shape_deviation = (
                torch.max(shape_deviation, 1.0 / shape_deviation).max(dim=2).values
            )
            ...
            xy_grid_inv = (
                feat_size - xy_grid
            )  # inverse just used for fast calculation of neighbor cell validity
            if neighbor == 0:
                pred_x, pred_y = xy_grid.long().T
                anchor_ind = (
                    pred_y * feat_w + pred_x
                ) * num_base_anchors + base_anchor_ind
            else:
                x_left_ok, y_up_ok = ((xy_grid % 1.0 < 0.5) & (xy_grid > 1.0)).T
                x_right_ok, y_down_ok = (
                    (xy_grid_inv % 1.0 < 0.5) & (xy_grid_inv > 1.0)
                ).T
                if neighbor == 1:
                    neighbor_ok = torch.stack(
                        (
                            torch.ones_like(x_left_ok),
                            x_left_ok,
                            y_up_ok,
                            x_right_ok,
                            y_down_ok,
                    ...
                    if neighbor_ok.numel() > 0:
                        four_direction_distance = (
                            torch.cat((xy_grid, xy_grid_inv), dim=-1) % 1.0
                        )
                        direction_mask = (
                            four_direction_distance
                            == four_direction_distance.min(dim=-1).values[:, None]
                        )
            ...
            indices.append((img_all_id[gt_ind], anchor_ind, gt_ind))
