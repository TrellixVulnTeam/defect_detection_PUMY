...
class SSDAnchorGenerator(AnchorGenerator):
    ...
    Args:
        ...
        ratios (list[float]): The list of ratios between the height and width
            of anchors in a single level.
        basesize_ratio_range (list(float)): Ratio range of anchors.
    ...
    def __init__(
        self, strides, ratios, basesize_ratio_range, input_size=300, scale_major=True
    ):
        assert len(strides) == len(ratios)
        assert is_list_of(basesize_ratio_range, float)
...
class LegacyAnchorGenerator(AnchorGenerator):
    ...
    Args:
        ...
        scale_major (bool): Whether to multiply scales first when generating
            base anchors. If true, the anchors in the same row will have the
            same scales. By default it is True
        octave_base_scale (int): The base scale of octave.
        scales_per_octave (int): Number of scales for each octave.
            `octave_base_scale` and `scales_per_octave` are usually used in
            retinanet and the `scales` should be None when they are set.
...
class LegacySSDAnchorGenerator(SSDAnchorGenerator, LegacyAnchorGenerator):
    ...
    def __init__(
        ...
        super(LegacySSDAnchorGenerator, self).__init__(
            strides, ratios, basesize_ratio_range, input_size, scale_major
        )
