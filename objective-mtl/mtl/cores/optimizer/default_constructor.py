...
from ..core_optimizer import OPTIMIZER_BUILDERS, OPTIMIZERS
...
def get_comm_param_groups(model, skip_list=(), skip_keywords=()):
    has_decay = []
    no_decay = []
    has_decay_name = []
    no_decay_name = []
...
class DefaultOptimizerConstructor:
    ...
    - ``custom_keys`` (list of dict): Specified parameters-wise settings by keys. If
      one of the keys in ``custom_keys`` is a substring of the name of one
      parameter, then the setting of the parameter will be specified by
      ``custom_keys[0][key]`` and other setting like ``bias_lr_mult`` etc. will
      be ignored. It should be noted that the aforementioned ``key`` is the
      longest key that is a substring of the name of the parameter. If there
      are multiple matched keys with the same length, then the key with lower
      alphabet order will be chosen.
      ``custom_keys[0][key]`` should be a dict and may contain fields ``lr_mult``
    ...
    def __init__(self, optimizer_cfg, paramwise_cfg=None):
        ...
        self.base_ld = optimizer_cfg.get("layer_decay", None)
        self.model_type = optimizer_cfg.get("model_type", None)
        self.model_depth = optimizer_cfg.get("model_depth", None)
        self.use_zero = optimizer_cfg.pop("use_zero", False)
        print("Zero value:", self.use_zero)
        self._validate_cfg()
    ...
    def _validate_cfg(self):
        ...
        if "custom_keys" in self.paramwise_cfg:
            # for easily update with task setting
            if not isinstance(self.paramwise_cfg["custom_keys"][0], dict):
                raise TypeError(
                    "If specified, custom_keys must be a list of dict, "
                    f'but got {type(self.paramwise_cfg["custom_keys"][0])}'
                )

            if self.base_wd is None:
                for key in self.paramwise_cfg["custom_keys"][0]:
                    if "decay_mult" in self.paramwise_cfg["custom_keys"][0][key]:
        ...
        if (
            ...
            or "layer_decay" in self.paramwise_cfg
        ):
            if self.base_wd is None:
                raise ValueError("base_wd should not be None")
    ...
    def add_params(self, params, module, prefix="", is_dcn_module=None):
        ...
        sorted_keys = sorted(sorted(custom_keys[0].keys()), key=len, reverse=True)
        ...
        for name, param in module.named_parameters(recurse=False):
            ...
            for key in sorted_keys:
                if key in f"{prefix}.{name}":
                    is_custom = True
                    lr_mult = custom_keys[0][key].get("lr_mult", 1.0)
                    param_group["lr"] = self.base_lr * lr_mult
                    if self.base_wd is not None:
                        decay_mult = custom_keys[0][key].get("decay_mult", 1.0)
    ...
    def add_layer_decay_param_groups(self, params, model, prefix=""):
        # get param-wise options
        custom_keys = self.paramwise_cfg.get("custom_keys", [{}])
        # first sort with alphabet order and then sort with reversed len of str
        sorted_keys = sorted(sorted(custom_keys[0].keys()), key=len, reverse=True)

        num_layers = sum(self.model_depth) + 2
        scales = list(self.base_ld ** i for i in reversed(range(num_layers)))
        if self.model_type == "swin":
            get_layer_func = partial(
                get_swin_layer, num_layers=num_layers, depths=self.model_depth
            )
        elif self.model_type == "vit":
            get_layer_func = partial(get_vit_layer, num_layers=num_layers)
        else:
        ...
        for name, param in model.named_parameters(recurse=False):
            ...
            for key in sorted_keys:
                if key in f"{prefix}.{name}":
                    ...
                    group_name = "%s_custom" % (key)
                    if group_name not in parameter_group_names:
                        parameter_group_names.append(group_name)
                        parameter_group_vars[group_name] = {
                            "weight_decay": self.base_wd * decay_mult,
                            "params": [],
        ...
        params.extend(list(parameter_group_vars.values()))

        for child_name, child_mod in model.named_children():
            child_prefix = f"{prefix}.{child_name}" if prefix else child_name
            self.add_layer_decay_param_groups(params, child_mod, prefix=child_prefix)
    ...
    def __call__(self, model):
        ...
        if self.base_ld is not None:
            params = []
            self.add_layer_decay_param_groups(params, model)
            optimizer_cfg.pop("layer_decay")
            optimizer_cfg.pop("model_type")
            optimizer_cfg.pop("model_depth")
            optimizer_cfg["params"] = params
        elif self.paramwise_cfg:
            params = []
        ...
        if self.use_zero:
            optimizer_class = OPTIMIZERS.get(optimizer_cfg.pop("type"))
            ...
            if self.paramwise_cfg:
                ...
                for param in params:
                    if len(param) == 1:
                        default_params.extend(param["params"])
                    else:
                ...
                return ZeroRedundancyOptimizer(
                    params=params, optimizer_class=optimizer_class, **optimizer_cfg
                )
        else:
