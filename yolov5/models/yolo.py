...
class Model(nn.Module):
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            if isinstance(m, (Conv, DWConv, CondConv)) and hasattr(m, 'bn'):
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 ...
                 CondConv, DynamicConv, TripletAttention, MHSA, CoAtNet,
                 CoordAtt, CoTAttention, ECAAttention, HaloAttention, MobileViTAttention, C3_ResNet, C3_ResNeXt, C3_Res2Net,
                 C3_ResNet_SE_TripletAttention, C3_ResNet_CoTAttention, C3_ResNet_SE, C3_ResNeXt_SE, CBAM, C3_ResNet_TripletAttention,
                 Shuffle_Block, conv_bn_relu_maxpool, C3_SE
                 ]:
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost, TripletAttention]:
